...
export default function App() {
  ...
  const processFrame = useCallback(() => {
    ...
    try {
      ...
      // Auto-stop every 7 seconds if enabled
      if (autoTranslate && Date.now() - recordingStartTimeRef.current > 7000) {
        handleTranslation();
      ...
      <main className="max-w-7xl mx-auto px-6 py-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <div className="glass-panel p-5 neon-border-pink space-y-3">
              <label className="text-[10px] font-mono opacity-50 uppercase tracking-widest flex items-center gap-2">
                <Activity className="w-3 h-3 text-cyber-pink" /> Auto-Translate
              </label>
              <div className="flex items-center justify-between bg-white/5 p-2 rounded-xl border border-white/10">
                <span className="text-[10px] font-mono opacity-70">{autoTranslate ? "ON (7s)" : "OFF"}</span>
                <button 
                  onClick={() => setAutoTranslate(!autoTranslate)}
                  className={cn(
                    "w-10 h-5 rounded-full transition-all duration-300 relative",
                    autoTranslate ? "bg-cyber-pink" : "bg-white/20"
                  )}
                >
                  <div className={cn(
